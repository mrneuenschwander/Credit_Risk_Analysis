...
### Results
Balanced Accuracy scores will be listed in descending order by model, along with their associated Classification Reports. The first image is the Accuracy Score, in the second image we are concerned with the 'rec' column. 'Rec' or 'recall' is the model's accuracy at determining if the data falls into one of those two categories.
...
The recall (or accuracy at predicting) is above 90% for both high and low risk categories, and performs solidly across other metrics as well. As will be seen as a theme here, the High_risk data has an extremely low precision rating ('prc'), which is indicative of higher sensitivity. When catching non-qualified buyers, higher sensitivity is preferred to precision, so as to not issue a loan to an individual that will not be able to pay it back within the terms of the loan agreement. Should a decline be issued in error, a second application would assist the model by providing data to show which applications may actually qualify that did not on the first time around, as well as have the opportunity to be discussed with an underwriter to circumvent the algo based on cicumstances.
